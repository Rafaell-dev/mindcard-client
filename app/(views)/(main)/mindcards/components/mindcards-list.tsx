...
import { MindcardCard } from "./mindcard-card";
import { PracticeHeader } from "./practice-header";
import { Mindcard } from "@/app/api/v1/mindcard/types";
...
interface MindcardsListProps {
  initialMindcards: Mindcard[];
}

export function MindcardsList({ initialMindcards }: MindcardsListProps) {
  const [searchQuery, setSearchQuery] = useState("");

  const mindcards = useMemo(() => {
    const mapped = initialMindcards.map((card, index) => {
